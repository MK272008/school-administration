...
export default function DashboardTableRow(props: BaseProps) {
	return (    
		<div className={`grid grid-cols-[5%_25%_30%_30%__10%] border-dark-border rounded-md hover:bg-dark-secondary transition-all cursor-pointer py-4 ${props.className}`}>
            {props.children}
        </div>
	);
}
...
export function DashboardTableColumn(props: DashboardTableColumnProps) {
    return (
        <span {...props} className={`my-auto mr-auto ${props.className}`}>
            {props.children}
        </span>
    );
}
